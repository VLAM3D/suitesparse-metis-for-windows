...
)
{
    ...
    gpu_p->d_Lx[1] = (unsigned char*)Common->dev_mempool + Common->devBuffSize;
    gpu_p->d_C = (unsigned char*)Common->dev_mempool + 2*Common->devBuffSize;
    gpu_p->d_A[0] = (unsigned char*)Common->dev_mempool + 3*Common->devBuffSize;
    gpu_p->d_A[1] = (unsigned char*)Common->dev_mempool + 4*Common->devBuffSize;
    gpu_p->d_Ls = (unsigned char*)Common->dev_mempool + 5*Common->devBuffSize;
    gpu_p->d_Map = (unsigned char*)gpu_p->d_Ls + (nls+1)*sizeof(Int) ;
    gpu_p->d_RelativeMap = (unsigned char*)gpu_p->d_Map + (n+1)*sizeof(Int) ;
